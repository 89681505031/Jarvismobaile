const state={mode:'phone',persona:localStorage.getItem('jarvisPersona')||'J.A.R.V.I.S.',waitingForCommand:false,diagnosticActive:false};
...
function onSpeechResult(text){const clean=(text||'').trim();if(!clean){showMessage('Не удалось распознать речь.');return}const wake=/^(джарвис|джервис|жарвис|jarvis|астра|astra|луна|luna|сайбер|cyber|терра|terra)(?=[,\s.!?]|$)[,\s.!?]*/i;const matched=clean.match(wake);const activation=(matched?.[1]||'').toLowerCase().replace(/^(джервис|жарвис)$/,'джарвис');const onlyWake=!!matched&&!clean.replace(wake,'').trim();if(activation)activatePersona(activation);if(onlyWake){state.waitingForCommand=true;showMessage('Слушаю… Персонаж: '+state.persona);window.AndroidJarvis?.speak('Слушаю');return}if(state.waitingForCommand){sendCommand(matched?clean.replace(wake,'').trim():clean);return}if(!matched)return;const rest=clean.replace(wake,'').trim();if(!rest){state.waitingForCommand=true;showMessage('Слушаю… Персонаж: '+state.persona);window.AndroidJarvis?.speak('Слушаю');return}sendCommand(rest)}
